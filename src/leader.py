...
class LeaderWork:
    ...
    def _carregar_dataset(self):
        # Carrega o dataset Breast Cancer da sklearn para validação real
        from sklearn.datasets import load_breast_cancer
        data = load_breast_cancer()
        X = data.data
        # O target do Breast Cancer já vem em formato int (0 ou 1)
        y = data.target
    ...
    def run(self):
        ...
        # 1. Carrega/Distribui o dataset apenas se for a liderança inicial (epoch == 0)
        ...
        while True:
            ...
            # Coleta resultados de forma não-bloqueante
            with self.context.lock:
                ctx = self.context.leader_context
